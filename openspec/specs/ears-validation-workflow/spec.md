...
This capability defines the EARS validation workflow for functional requirements in the IEEE 830 document, providing a structured checklist per subsection that verifies keyword presence (WHEN, THE SYSTEM SHALL) and tracks state transitions from `proposed` to `approved`.
